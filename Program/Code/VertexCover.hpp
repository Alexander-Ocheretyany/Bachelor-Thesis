...
public:
	...
	VertexCover(size_t s, std::vector<std::list<size_t>> g) {
		...
		VC = findVC(graph, 0, 1, VC, size); // Vertex cover
...
private:
	...
	std::vector<size_t> findVC(std::vector<std::list<size_t>> aux, size_t deletedVertex, bool first, std::vector<size_t> accumulator, size_t k) {
		...
		if (edge) { // If there is an edge in the graph
			acc_1 = findVC(aux, v, 0, accumulator, k - 1); // Left child
			acc_2 = findVC(aux, aux[v].front(), 0, accumulator, k - 1); // Right child
		}
		...
		if (acc_1.size() == 0 && acc_2.size() == 0) { // If both children are empty
			return std::vector<size_t>();
		}
		else if (acc_1.size() == 0 && acc_2.size() != 0) { // If only left child is empty
			for (auto x : acc_2) {
				accumulator.push_back(x);
			}
		}
		else if (acc_2.size() == 0 && acc_1.size() != 0) { // If only right child is empty
			for (auto x : acc_1) {
				accumulator.push_back(x);
			}
		}
		...
		else if (acc_2.size() < acc_1.size()) { // If the number of vertices in the accumulator of the right child is smaller
			for (auto x : acc_2) { 
				accumulator.push_back(x);
			}
		}
