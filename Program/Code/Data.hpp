...
public:
	...
	// void error(std::string str) - prints out str and finishes the program
...
private:
	...
	bool initialDialogue() {
		...
		case 'r':
		{
			...
			{
				int VCsize = 0;
				std::cout << "Size of the vertex cover (0 if not known): ";
				std::cin >> VCsize;
				std::cout << std::endl;
				if (VCsize == 0) {
					VCsize = graph.size();
					...
					Data::error("Wrong size of the vertex cover!");
				}
				std::cout << "Finding of the VC and the IS...";
				VertexCover cover(VCsize, graph); // Generate vertex cover
				VC = cover.getVertexCover(); // Vertex cover

				if (VC.size() == 0) {
					Data::error("VC of the given size cannot be found!");
				}
