...
public:	
	...
	void fillRandomly(size_t normal, size_t max) {
		...
		size_t minimum = length / max;
		size_t index = 0;

		for (size_t i = 0; i < length; ++i) {
			if (i != 0 && i % minimum == 0) ++index;
			if (index < max) ind.push_back(index);
			else break;
		}

		// Correctly fill in the individual
		index = 0;
		while (ind.size() < length) ind.push_back(index++);

		// Randomize
		std::random_shuffle(ind.begin(), ind.end()); // Permute
